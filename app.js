...
function broadcast(data) {
    app.ws.server.clients.forEach(function each(client) {
        if (client.readyState === 1) {
            let registerOps = client.registerOps;
            if (!registerOps || !data.body.repository) {
                console.log('not register', registerOps);
                client.send(JSON.stringify(data));
                return;
            }
            let remoteBranch = data.body.ref && data.body.ref.replace('refs/heads/', '');
            let localBranch = registerOps.branch;
            let remoteRepositoryName = data.body.repository.name;
            let localRepositoryName = registerOps.repository;
...
app.ws.server.on('connection', function connection(ws) {
    ...
    ws.on('message', function incoming(data) {
        ...
        if (data.action === 'reg') {
            ...
            console.log('client 注册成功:', ws.registerOps);
        }
        // console.log(ws,data);
    });
    ...
    ws.on('close', function incoming(data) {
        // Broadcast to everyone else.
        console.log('client 连接断开:', ws.registerOps);
    });
